...
data class UserProfile(
    ...
    /** Male lineage — mutually exclusive with [isLevi] in the UI. */
    val isKohen: Boolean = false,
    /** Male lineage — mutually exclusive with [isKohen] in the UI. */
    val isLevi: Boolean = false,
    /** User is the mother's firstborn son — may need his own Pidyon HaBen. */
    val isFirstbornSon: Boolean = false,
    /** User has a son who may need Pidyon HaBen (father's perspective). */
    val hasFirstbornSon: Boolean = false,
    /**
     * Checklist item ids the user chose to hide permanently (e.g. Pidyon HaBen after a rav
     * confirmed it does not apply). Survives across days and reinstalls of the checklist catalog.
     */
    val hiddenForeverItemIds: List<String> = emptyList(),
    ...
    /** Pidyon HaBen — firstborn flags qualify unless user is Kohen or Levi (they are exempt). */
    val showsFirstbornChecklistItems: Boolean
        get() = (isFirstbornSon || hasFirstbornSon) && !isKohen && !isLevi
...
data class ChecklistItemDef(
    ...
    /** When true, item appears only if [UserProfile.isKohen] is set. */
    val kohen: Boolean? = null,
    /** When true, item appears only if [UserProfile.isLevi] is set. */
    val levi: Boolean? = null,
    /** When true, item appears only if [UserProfile.isFirstbornSon] or [UserProfile.hasFirstbornSon] is set. */
    val firstborn: Boolean? = null,
