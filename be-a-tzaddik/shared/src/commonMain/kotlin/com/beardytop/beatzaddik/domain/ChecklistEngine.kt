...
class ChecklistEngine(
    ...
    private fun matches(
        ...
        if (item.kohen == true && !profile.isKohen) return false
        if (item.levi == true && !profile.isLevi) return false
        if (item.firstborn == true && !profile.showsFirstbornChecklistItems) return false
        if (item.id in profile.hiddenForeverItemIds) return false
