...
fun HalachicClickableText(
    ...
    val underlineStrokePx = with(density) { 2.dp.toPx() }
    val underlineOffsetPx = with(density) { 3.dp.toPx() }
    ...
    fun handleTap(offset: Int) {
        annotated.getStringAnnotations(HalachicTermsDictionary.annotationTag(), offset, offset)
            .firstOrNull()
            ?.item
            ?.let { id ->
                HalachicTermsDictionary.termById(id) ?: HalachicGuideTerms.termById(id)
            }
            ?.let(showTerm)
