...
object ShabbatGuideData {
    ...
    val glossary = listOf(
        GuideTopic(
            ...
            hebrewTitle = "שַׁבָּת",
            body = """Shabbat holds a monumental place in Jewish tradition as the fourth of the Ten Commandments, given to us as a beautiful sign from Hashem. It is a weekly twenty-five-hour sanctuary in time, spanning from Friday sunset to Saturday night, designed to pull us away from the exhausting grind of the workweek. Rather than feeling like a list of restrictions, this day is a profound gift that allows us to step back from our attempts to master the physical world. By unplugging from screens, avoiding driving, and pausing the constant impulse to produce and create, we experience a deep sense of spiritual elevation, stepping into a peaceful oasis where we can connect genuinely with Hashem, our families, and our own souls. By observing its laws properly, ideally we should become so spiritually elevated that we can sense that the whole universe is simply Hashem's creation.
...
Rosh Chodesh is a semi-holiday — certain fasting and eulogizing are restricted. There is a widespread custom for women to refrain from certain types of work as an extra mark of honor for the day."""
        ...
        GuideTopic(
            ...
            // Aligned with ErevChagPrepText.festivalYomTovLawsBlock + YomTovShabbatPrepText briefs
...
Ochel nefesh vs Shabbat:
Preparing for a biblical festival is similar to preparing for Shabbat, but with critical differences. Shabbat is absolute rest; Yom Tov is physical, communal celebration. The Torah therefore permits certain food-related melacha on Yom Tov — ochel nefesh ("food for sustenance") — that Shabbat forbids (Exodus 12:16): cooking and baking for that day's meals from a pre-existing flame, and carrying for holiday needs even without an eruv.
...
Hachana (meechin) — no prep across days:
You may not do physical labor or tasks on Day 1 for Day 2, or for after Shabbat/Yom Tov (meal prep, washing plates, setting the table for tomorrow, and similar). Prep for the next night's meal waits until tzeit hakochavim (nightfall) when the first day ends. Loophole: if you are cooking food to eat today, you may cook extra in that same pot — without extra physical effort or saying it is for tomorrow.
...
• Yom Tov on Friday leading into Shabbat: make eruv tavshilin before Yom Tov begins (blessing + declaration over cooked food and a baked item — see Eruv Tavshilin in this guide). That lets you finish Shabbat food prep on Friday Yom Tov. Light Shabbat candles Friday before sunset from a pre-existing flame.
• Shabbat falling directly before Yom Tov: no eruv tavshilin. You cannot prepare anything on Shabbat for Yom Tov — wait until Shabbat ends completely. After nightfall, Havdalah is inside Kiddush (Yaknehaz: wine → Kiddush → candle → Havdalah → Shehecheyanu; no spices). Light Yom Tov candles only from a flame lit before Shabbat.
...
See the Shabbat vs Yom Tov comparison table in this guide for a side-by-side summary. Always consult your rabbi for details."""
        ...
        GuideTopic(
            ...
            hebrewTitle = "עֵרוּב תַּבְשִׁילִין",
            body = """Eruv tavshilin is needed only in years when Yom Tov is immediately followed by Shabbat. Without it, you may not cook on Yom Tov for Shabbat (hachana / meechin). It also reminds the household to prepare for Shabbat amid festival rush.

When: set it aside on Erev Yom Tov, before the festival begins (before candle lighting). If two days of Yom Tov lead into Shabbat, make it before the first day starts. It is not made when Shabbat falls before Yom Tov.

How (brief):
• Set aside two foods: a baked item (whole challah or matzah) and a cooked dish (meat, fish, or a hard-boiled egg with the shell on are common). The cooked dish is essential; both together is ideal; baked alone does not work.
• Recite the blessing and eruv declaration from your Machzor or siddur, in any language you understand. One eruv per household is enough.
• Keep at least a kezayit of the cooked food until Shabbat cooking is finished. Store in a safe, clearly labeled spot — if the baked item is eaten by mistake the eruv remains valid; if the cooked dish is destroyed before Friday cooking is done, ask your rav.

Limits:
• Permits cooking and food prep on Yom Tov for Shabbat only — not cooking on one day of Yom Tov for the next festival day.
• Start Friday cooking early enough that the food could theoretically be eaten before Shabbat begins.
• Does not permit other melacha that remains forbidden on Yom Tov (writing, new fire, electricity, etc.).

If you forgot to make it: ask your rabbi immediately what you may still prepare for Shabbat."""
        ),
        GuideTopic(
            id = "muktzeh",
            title = "Muktzeh",
...
• When in doubt about any situation, ask your rabbi.""",
    ...
    // Brief side-by-side of the core rules also covered in the Yom Tov / Eruv Tavshilin
    // glossary topics and in the seasonal Yom Tov–Shabbat explainers.

    data class ComparisonRow(val activity: String, val shabbat: String, val yomTov: String)

    // Source: chabad.org/library/article_cdo/aid/708510/jewish/Laws-of-Yom-Tov.htm
    // + ErevChagPrepText.festivalYomTovLawsBlock
    val shabbatYomTovComparison = listOf(
        ComparisonRow(
            "Cooking & baking",
            "Forbidden — food fully cooked before Friday sunset",
            "Permitted for that day's meals from a pre-existing flame (ochel nefesh)"
        ),
        ComparisonRow(
            "Reheating / plata",
            "Dry, fully cooked food only — reheat with a shinui (unusual method), e.g. upside-down on blech or inside another vessel. Food on blech from before Shabbat may stay. Ashkenazim generally do not warm cold liquids.",
            "Cold liquids (soup, etc.) and dry food may be reheated on a plata for that day's meals — part of simchat Yom Tov"
        ),
        ComparisonRow(
            "Kindling a flame",
            "Forbidden (no new fire, no transferring)",
            "Transfer only from a pre-existing flame (transfer candle lit before Yom Tov). No match/lighter once Yom Tov began"
        ),
        ComparisonRow(
            "Raising / lowering a flame",
            "Forbidden",
            "Raising an existing gas flame to cook is generally OK; lowering or extinguishing is generally forbidden (narrow grama only per your rav)"
        ),
        ComparisonRow(
        ...
        ComparisonRow(
            ...
            "Permitted for Yom Tov needs (siddur, keys, stroller, food to a meal, etc.)"
        ),
        ComparisonRow(
            "Hachana / meechin (prep for next day)",
            "Forbidden — no prep on Shabbat for after Shabbat or for Yom Tov",
            "Forbidden — prep only for that Yom Tov day; wait until nightfall for Day 2. Extra in today's pot OK if no extra effort / no 'for tomorrow'"
        ),
        ComparisonRow(
            "Yom Tov → Shabbat (Friday YT)",
            "N/A",
            "Eruv tavshilin before Yom Tov begins — then finish Shabbat food prep on Friday YT; light Shabbat candles before sunset from existing flame"
        ),
        ComparisonRow(
            "Shabbat → Yom Tov",
            "No eruv tavshilin — wait until Shabbat ends before any Yom Tov prep",
            "After tzeit: Yaknehaz Kiddush–havdalah; light Yom Tov candles from pre-existing flame only"
        ),
        ComparisonRow(
            "Yom Tov on Saturday",
            "Full Shabbat laws",
...
The Seder is held after nightfall on 15 Nissan (and the second night in the Diaspora). No chametz may be eaten after the morning deadline on Erev Pesach.""",
    ...
    val termAnchorMap = mapOf(
        ...
        "Yom Tov" to "yom_tov",
        "Eruv Tavshilin" to "eruv_tavshilin",
        "eruv tavshilin" to "eruv_tavshilin",
        "Ochel nefesh" to "yom_tov",
        "Hachana" to "yom_tov",
        "Meechin" to "yom_tov",
        "Yaknehaz" to "havdalah",
