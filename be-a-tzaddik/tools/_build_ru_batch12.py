#!/usr/bin/env python3
"""Build _ru_top50_batch12_data.py from _ru_batch12_manual.json."""
...
MANUAL_JSON = Path(__file__).parent / "_ru_batch12_manual.json"
OUT = Path(__file__).parent / "_ru_top50_batch12_data.py"
...
def main() -> None:
    seed = Path(__file__).parent / "_seed_batch12_manual.py"
    if not MANUAL_JSON.exists():
        subprocess.run([sys.executable, str(seed)], check=True)
    if not CAND.exists():
        subprocess.run([sys.executable, str(Path(__file__).parent / "_gen_batch12_candidates.py")], check=True)
    ...
    lines = [
        '"""Batch 12: next 50 Latin-gap keys — explicit mappings."""',
        "",
        "from __future__ import annotations",
        "",
        "TOP50_BATCH12: dict[str, str] = {",
    ]
    for k in keys:
