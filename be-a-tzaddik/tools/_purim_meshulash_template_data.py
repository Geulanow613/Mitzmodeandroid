"""English catalog keys for Purim Meshulash explainers (must match PurimMeshulashText.kt)."""

from __future__ import annotations

MEGILLAH_BLESSINGS_COMMON = """Blessings before reading:
• Al mikra megillah
• She'asa nissim
• Shehecheyanu (recited on the first night; Ashkenazim recite it by day as well)"""

FULL_SCHEDULE = """Purim Meshulash (פורים מְשֻׁלָּשׁ) — Jerusalem when Shushan Purim (15 Adar) falls on Shabbat
...
ADVANCE_PREP = """Tomorrow is erev Purim — and this year is Purim Meshulash in Jerusalem. Shabbat is in the middle, so you need the full plan now (not only tomorrow).

$scheduleBlock"""

EREV_PREP = """Purim Meshulash starts tonight in Jerusalem. Because Shabbat falls in the middle of the festival, read and save this plan now — you will not be able to rely on the app on Shabbat for Sunday's mitzvot.
...
FRIDAY_MEGILLAH = f"""Purim Meshulash — Megillah on Friday (14 Adar)
...
FRIDAY_MATANOT = """Purim Meshulash — matanot la'evyonim on Friday only (14 Adar)
...
SUNDAY_MISHLOACH = """Purim Meshulash — mishloach manot on Sunday (16 Adar)
...
SUNDAY_SEUDAH = """Purim Meshulash — Purim seudah on Sunday (16 Adar)
...
PURIM_MESHULASH_CATALOG_KEYS: list[str] = [
    MEGILLAH_BLESSINGS_COMMON,
    FULL_SCHEDULE,
    ADVANCE_PREP,
    EREV_PREP,
