# -*- coding: utf-8 -*-
"""Clean Hebrew translations for mitzvot_003 entries 6-25 (cloud51-cloud75)."""
...
NEW_ENTRIES: list[str] = [
    ...
    (
        'קבעu פגישah לqהל פרטi עם מlach המlachim! 👑 שemooneh esrei — הידוע גamכi בעamidah — '
        'הוא לb כl תpילah hayehudit. חכmינu חibru אותah ברuach hakodesh, כלומר כl מילה היא מפתch '
        'מדויק שנועד לפתoach שערים shamayim ולהorid ברכot ruhaniot ve-gashmiot! 🔓 '
        'כi זu שיחah ישירah עם ה\', להlכah יש "כללי פרוטוקול מlכuתi" לשמור על ריכוז. '
        'קודם — לעמוד ברגליים צמודות, בחיקוי המלאכים שיש להם "רגל אחת", '
        'להראות מסירות מוחלטת (ושאתם לא הולכים לשום מקום בזמן שמדברים עם המlach!). 😇 '
        'גam פונים לירושלים — איפה שאתם בעולm — כדי לכוון את הלב לעבר מקום בית המקdash! 🏛️ '
        'התpילah מחולקת לשלושah חלקים: שלוש ברכot שבח, שלוש עשרה בקשot לדברים כמו חכמה, '
        'בריאות ושgשוג, ושלosh ברכot סyום של הודיה. בשבת וchagim, פormat הברכot האמצעיות משתנה — '
        'ודאu שאתם קוראים את העamidah הנכונה מהסiddur! 📜 '
        'בעod שכוונה טובah לאורך כl התpילah, החכmim אומרים שהיא קריטית ביותר בברכah הראשונה "אבot" — '
        'אם איבדתם שם את הריכוז, פסpסתם את ליבת המpגsh (אshkenazim לא חוזרים לחזור, סfardim כן)! 🕊️ '
        'היכנסu למmmד אחר והורidu את כl הברכot שה\' רוצה לתת לכם! למdו עod! 🎊🙏 '
        'מקור: תלmoud, ברachot ל"ד ע"ב; שulchan aruch, orach chaim צ"ה-צ"ז.'
    ),
]
